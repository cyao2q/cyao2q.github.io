function btnFan() {
    ...
    if (txt) {
        var regs = txt.split(' ');
        var fields = ["second", "min", "hour", "day", "mouth", "week"];

        // 设置值并初始化前6个字段
        for (var i = 0; i < 6; i++) {
            $("input[name=v_" + fields[i] + "]").val(regs[i] || "");
        }
...
function initMonth(strVal) {
    $(".mouthList input[type='checkbox']").prop("checked", false);
    var ary = null;
...
function initWeek(strVal) {
    ...
    if (strVal == "*") {
        ...
        ary = strVal.split(",");
        for (var i = 0; i < ary.length; i++) {
            $(".weekList input[value='" + ary[i] + "']").prop("checked", true);
        }
    }
}
...
function cycle(dom) {
    var name = dom.name;
    var ns = $(dom).parent().find("input[type='number']");
...
$(function () {
    $(".line input[type='number']").on("change", function () {
    ...
    $("#mouth_appoint").click(function () {
        if (this.checked) {
            if ($(mouthList).filter(":checked").length == 0) {
                $(mouthList.eq(0)).prop("checked", true);
            }
